...
function M.generateResults( conOutput )
    ...
    if summaryCnt > 1 then
        local summaryFp = io.open( g_localStoreDir.."summary.txt", "w" )
        summaryFp:write(string.rep("=", 100).."\n")
        summaryFp:write(string.format("%-30s    %8s   %8s  %8s    %8s    %8s",
              "SUMMARY","TotalLines","Commented","Code","Executed","Coverage" ).."\n")
        for filename, tResults in pairs(summary) do
            summaryFp:write(string.format("%-30s    %7d       %7d    %7d    %7d    %f",
                              filename,
                              tResults.total,
                              tResults.commented,
                              tResults.actualCode,
